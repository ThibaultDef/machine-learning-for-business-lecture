...
\frac{1}{n}
\sum_{i=1}^n
\left(y_i - w_0 - \sum_{j=1}^p w_j x_{ij}\right)^2
\]
...
\end{frame}
\begin{frame}f{What is the gradient $\nabla L(\theta)$?}

The gradient is a vector of partial derivatives. For a scalar loss function $L(\theta)$, it points in the direction of steepest increase.

\vspace{0.3cm}

\textbf{What is a derivative?}

A derivative measures how much a function changes when we perturb its input slightly:

\[
\frac{dL}{d\theta} = \lim_{\delta\theta \to 0} \frac{L(\theta + \delta\theta) - L(\theta)}{\delta\theta}
\]

This ratio tells us the rate of change of $L$ with respect to $\theta$.
